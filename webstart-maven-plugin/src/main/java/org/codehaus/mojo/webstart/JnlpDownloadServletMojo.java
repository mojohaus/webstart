...
public class JnlpDownloadServletMojo extends AbstractBaseJnlpMojo {
    ...
    private void checkJnlpFileConfiguration(JnlpFile jnlpFile) throws MojoExecutionException {
        ...
        //        {
        //            verboseLog(
        //                "No templateFilename found for " + jnlpFile.getOutputFilename() + ". Will use the default
        // template." );
        //        }
        //        else
        //        {
     ...
     */
    private Set<ResolvedJarResource> resolveJarResources(
            Collection<JarResource> configuredJarResources, Set<ResolvedJarResource> commonJarResources)
        ...
        if (!isExcludeTransitive()) {
            ...
            Set<Artifact> transitiveArtifacts = getArtifactUtil()
                    .resolveTransitively(
                            artifacts,
                            siblingProjects,
        ...
        for (ResolvedJarResource jarResource : collectedJarResources) {
            ...
            boolean copied = copyJarAsUnprocessedToDirectoryIfNecessary(
                    artifact.getFile(), getLibDirectory(), filenameWithVersion);
            ...
            String filename = getDependencyFilenameStrategy()
                    .getDependencyFilename(
                            artifact, jarResource.isOutputJarVersion() ? null : false, isUseUniqueVersions());
    ...
    private void generateJnlpFile(ResolvedJnlpFile jnlpFile, String libPath) throws MojoExecutionException {
        ...
        GeneratorTechnicalConfig generatorTechnicalConfig = new GeneratorTechnicalConfig(
                ...
                getEncoding());
        JarResourceGeneratorConfig jarResourceGeneratorConfig = new JarResourceGeneratorConfig(
                jarResources, libPath, getCodebase(), jnlpFile.getProperties(), jnlpFile.getArguments());
