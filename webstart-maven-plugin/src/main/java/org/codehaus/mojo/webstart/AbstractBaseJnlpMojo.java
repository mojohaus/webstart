...
public abstract class AbstractBaseJnlpMojo extends AbstractMojo {
	 ...
	 * resources that are to be included in the JNLP bundle.
	 */
	...
	/**
	 * Indicates whether or not gzip archives will be created for each of the
	 * jar files included in the webstart bundle.
	 */
	 ...
	 * <p/>
	 * <strong>Note:</strong> If this property is not defined, then will use a
	 ...
	 * <p/>
	 * If set to false and the {@code unsign} parameter is set to {@code true}
	 * then the build will fail if there is a jar to unsign, to avoid this use
	 * then the extension jnlp component.
	 *
	 * @since 1.0-beta-2
	 */
	 ...
	 * <p/>
	 * Since jdk 1.7u45, you need to add some entries to be able to open jnlp
	 * files in High security level. See
	 * http://www.oracle.com/technetwork/java/javase/7u45-relnotes-2016950.html
	 * <p/>
	 * <strong>Note:</strong> Won't affect any already signed jar resources if
	 * you configuration does not authorize it.
	 * <p/>
	 ...
	 * <li><strong>full</strong>: groupId-artifactId[-classifier]-version.jar
	 * </li>
	 * </ul>
	 *
	...
	/**
	 * Use unique version for any snapshot dependency, or just use the
	 * {@code -SNAPSHOT} version suffix.
	 *
	 * @since 1.0-beta-7
	 */
	 ...
	 * <p/>
	 * We use a plexus list injection instead of a direct component injection
	 * since for a jre 1.4, we will at the moment have no implementation of this
	 * tool.
	 * <p/>
	 * Later in the execute of mojo, we will check if at least one
	 * implementation is available if required.
	 *
	 * @since 1.0-beta-2
	 */
	...
	// the jars to sign and pack are selected if they are prefixed by
	// UNPROCESSED_PREFIX.
	...
	// We achieve that by only filtering files modified after the plugin was
	// started
	// Note: if other files (the pom, the keystore config) have changed, one
	// needs to clean
	 ...
	 * directory, where the libraries are placed. If the path is not configured
	...
	/**
	 * Returns the flag that indicates whether or not jar resources will be
	...
	/**
	 * Returns the working directory. This is the directory in which files and
	 * resources will be placed in order to be processed prior to packaging.
	 *
	...
	/**
	 * Returns the library directory. If not libPath is configured, the working
	...
	/**
	 * Returns the location of the directory containing non-jar resources that
	...
	/**
	 * Returns the file handle to the directory containing the Velocity
	 * templates for the JNLP files to be generated.
	 *
	...
	/**
	 * Returns the flag that indicates whether or not a gzip should be created
	 * for each jar resource.
	 *
	...
	/**
	 * Returns the flag that indicates whether or not all transitive
	 * dependencies will be excluded from the generated JNLP bundle.
	 *
	...
	/**
	 * Returns the collection of artifacts that have been modified since the
	 * last time this mojo was run.
	 *
		...
		if (getDependencyFilenameStrategy() == null) {
			...
			if (dependencyFilenameStrategy == null) {
				throw new MojoExecutionException("Could not find filenameMapping named '" + filenameMapping
						+ "', use one of the following one: " + dependencyFilenameStrategyMap.keySet());
			}
		}
	}

	/**
	 * Conditionally copy the jar file into the target directory. The operation
	 * is not performed when a signed target file exists and is up to date. The
	 * signed target file name is taken from the <code>sourceFile</code> name.E
	 * The unsigned target file name is taken from the <code>sourceFile</code>
	 * name prefixed with UNPROCESSED_PREFIX. TODO this is confusing if the
	 * sourceFile is already signed. By unsigned we really mean 'unsignedbyus'
	 *
	 * @param sourceFile
	 *            source file to copy
	 * @param targetDirectory
	 *            location of the target directory where to copy file
	 * @param targetFilename
	 *            [optional] to change the target filename to use (if
	 *            {@code null} will use the sourceFile name).
	 * @return <code>true</code> when the file was copied, <code>false</code>
	 *         otherwise.
	 * @throws IllegalArgumentException
	 *             if sourceFile is <code>null</code> or
	 *             <code>sourceFile.getName()</code> is <code>null</code>
	 * @throws MojoExecutionException
	 *             if an error occurs attempting to copy the file.
	 */
	 ...
	 *
	 * @throws MojoExecutionException
	 *             if can not sign or rename jars
	 */
	protected void signOrRenameJars() throws MojoExecutionException {
		...
		if (sign != null) {
			...
			if (isPack200()) {
				...
				// As out current Pack200 ant tasks don't give us the ability to
				// use a temporary area for
				// creating those temporary packing, we have to delete the
				// temporary files.
				ioUtil.deleteFiles(getLibDirectory(), unprocessedPack200FileFilter);
				// specs says that one should do it twice when there are
				// unsigned jars??
				// Pack200.unpackJars( applicationDirectory,
				// updatedPack200FileFilter );
			}
	 ...
	 *         jar:file:..../webstart-maven-plugin-.....jar!/
	 */
	...
	/**
	 * @param directory
	 *            location of directory where to update manifest entries jars
	 * @throws MojoExecutionException
	 *             if can not update manifest entries jars
	 */
	...
	/**
	 * @param directory
	 *            location of directory where to sign jars
	 * @return the number of signed jars
	 * @throws MojoExecutionException
	 *             if can not sign jars
	 */
			...
			while (!signRequests.isEmpty()) {
				for (int i = signRequests.size() - 1; i >= 0; i--) {
					if (signRequests.get(i).isDone()) {
						try {
							signRequests.get(i).get();
						} catch (InterruptedException e) {
							break;
						} catch (ExecutionException ee) {
							throw (MojoExecutionException) ee.getCause();
						}
	...
	/**
	 * Removes the signature of the files in the specified directory which
	 * satisfy the specified filter.
	 *
	 * @param workDirectory
	 *            working directory used to unsign jars
	 * @return the number of unsigned jars
	 * @throws MojoExecutionException
	 *             if could not remove signatures
	 */
	private int removeExistingSignatures(File workDirectory) throws MojoExecutionException {
		...
		for (File jarFile : jarFiles) {
			if (isJarSigned(jarFile)) {
				if (!canUnsign) {
					throw new MojoExecutionException("neverUnsignAlreadySignedJar is set to true and a jar file ["
							+ jarFile + " was asked to be unsign,\n please prefer use in this case an extension for "
